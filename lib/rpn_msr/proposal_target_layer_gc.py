...
DEBUG = False
#传入的数据为
# rpn_rois：经过roipooling层之后的blob，内容为[proposal引索(全零)，proposal]
...
#函数作用，产生两个（len(rois),4*21）大小的矩阵，其中一个对fg-roi对应引索行的对应类别的4个位置填上（dx,dy,dw,dh），另一个对fg-roi权重对应引索行的对应类别的4个位置填上（1,1,1,1）
...
#bbox_inside_weights（对fg-roi对应引索行的对应类别的权重的4个位置填上（1,1,1,1））
